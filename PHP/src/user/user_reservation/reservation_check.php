    ...
    <body>
        ...
        try{
            ...
            if(!is_numeric($number_of_user)){
                print '<br><br><br><br>';
                print'<p class="check">予約人数を半角数字で入力してください。</p>';
            ...
            if($check_numeric == "true"){
                if($number_of_user > $max_people){
                    print '<br><br><br><br>';
                    print'<p class="check">予約人数が適切ではありません。</p>';
                    print'<form method="post" action="reservation_add_01.php">';
                        print'<input type="hidden" name="room_ctg" value="'.$room_ctg.'">';
                        print'<p class="buttons"><input type="submit" class="button" value="予約画面に戻る"></p>';
                    print'</form>';
                    print'<form method="post" action="../../login/user_home.php">';
                        print'<p class="buttons"><input type="submit" class="button" value="ホーム画面に戻る"></p>s';
                    print'</form>';
            ...
            if($check_numeric == "true" && $check_max_people == "true" && $check_item_max == "true" && $check_time == "true"){
                ...
                if($rec==true)
                {
                    print '<p class="check">指定された時間帯は既に予約されています。</p>';
                    print'<form method="post" action="reservation_add_01.php">';
                        print'<input type="hidden" name="room_ctg" value="'.$room_ctg.'">';
                        print'<p class="buttons"><input type="submit" class="button" value="予約画面に戻る"></p>';
                    print'</form>';
                ...
                else
                {
                    ...
                    $sql_reservation_number = "SELECT COALESCE(MAX(reservation_number) + 1, 1) 
                                                AS new_reservation_number 
                                                FROM reservation_table"; 
                    $stmt2 = $dbh->prepare($sql_reservation_number); 
                    $stmt2->execute(); 
                    $result = $stmt2->fetch(PDO::FETCH_ASSOC); 
                    $reservation_number = $result['new_reservation_number'];//

                    $sql_tentative = "INSERT INTO reservation_table 
                                    (reservation_number, 
                                    user_Id, 
                                    room_number,
                                    number_of_user,
                            ...
                            VALUES (?,?,?,?,?,?)";//             
                    $stmt = $dbh->prepare($sql_tentative);
                    $stmt->execute([
                        $reservation_number,//
                        $_SESSION['user_Id'],
                        $room_number,
                        ...
                        /*$r_starttime,
                        $r_endtime*/
                        $r_starttime->format('Y-m-d H:i'),
                        $r_endtime->format('Y-m-d H:i')
                    ]);


                    //予約履歴テーブルに仮追加
                    $sql_tentative = "INSERT INTO history_table 
                                    (reservation_number, 
                                    user_Id, 
                                    room_name,
                                    number_of_user,
                    ...
                    if($use_ex=='Y')
                    {
                        ...
                        // 予約内容の表示
                        echo '<h2 class="check">この内容で予約しますか？</h2><br>';
                        echo '<p class="check">部屋名: ' . htmlspecialchars($room_name) . '</p>';
                        echo '<p class="check">利用人数: ' . htmlspecialchars($number_of_user) . '人</p>';

                    
                        echo '<p class="check">利用開始時間: ' . $r_starttime->format('Y-m-d H:i') . '</p>';
                        echo '<p class="check">利用終了時間: ' . $r_endtime->format('Y-m-d H:i') . '</p>';
                    
                        //貸出備品がある場合は表示
                        echo '<p class="check">貸出備品<br><br>';
                        if($item_name_data!=null){
                            ...
                            echo '<p class="check">なし</P>';
                        }
                    
                        echo '<p class="check">備考: ' . htmlspecialchars($remark) . '</p>';
                        ////////////////////////////////////////////////////////////////////////////////^^^^^^^^^^^^^^^^^^^^^^
                        if($use_ex == "Y"){
                            if($price_ex == "P"){
                                echo '<p class="check">施設料金: ' . number_format($facility_price) . '円</p>';
                            }
                            if($rental_ex == "R"){
                                echo '<p class="check">備品料金: ' . number_format($total_rental_price - $facility_price) . '円</p>';
                            }
                            if($price_ex == "P"){
                               echo '<p class="check">合計料金: ' . number_format($total_rental_price) . '円</p>'; 
                            }
                        }
                        ////////////////////////////////////////////////////////////////////////////////^^^^^^^^^^^^^^^^^^^^^^
...
/////////////////////////////////////////////////////////////////////////////^^^^^^^^^^^^^^^^^                                  
                            ...
                            <?php
                            print'<form method="post" action="reservation_add_stop.php">';
